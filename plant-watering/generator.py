...
def get_instance_name(size, run):
    return 'instance_{}_{}'.format(size, run)
...
class Problem(object):
    def __init__(self, name, domain, size, num_plants, num_taps=1):
        self.name = name
        self.domain = domain
        self.size = size
...
def Main():
    ...
    for size in range(4, args.max + 1):
        ...
        for run in range(1, 4):
            name = get_instance_name(size, run)
